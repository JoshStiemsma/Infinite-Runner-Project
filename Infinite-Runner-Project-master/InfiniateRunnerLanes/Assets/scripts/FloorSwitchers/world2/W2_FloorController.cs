...
public class W2_FloorController : MonoBehaviour {
    ...
    void Update()
    {
        if (player.transform.GetComponent<playercontroller>().health <= 0 && reset == false)
        {
            ResetFirstMesh();
            reset = true;
        }
        else if (player.transform.GetComponent<playercontroller>().health >= 1)
        {
            reset = false;
        }
